...
import { healthData } from '../data/healthData'; // Adjust path as needed
import { User } from 'lucide-react'; // Ensure lucide-react is installed
...
const AppointmentsView = () => {
  ...
  const filteredAppointments = filter === 'all'
    ...
    : healthData.appointments.filter(apt => apt.status === filter);
      ...
      <div className="bg-white rounded-lg p-4 mb-6">
        <div className="flex space-x-4">
          {['all', 'confirmed', 'pending', 'completed', 'cancelled'].map((status) => (
            ...
            >
              {status} ({status === 'all' ? healthData.appointments.length : healthData.appointments.filter(a => a.status === status).length})
            </button>
          ))}
        </div>
      </div>
